...
{
    /// <summary>
    /// Maximum size of the git diff output in characters, matching the Python/TypeScript SDKs.
    /// </summary>
    private const int MaxDiffChars = 65536;
    ...
    public static async Task<RepoInfo?> GetRepoInfoAsync(GitMetadataSettings? settings = null)
    {
        try
        {
            ...
            string? gitDiff = null;
            if (ShouldCollect(fields, "git_diff"))
            {
                // Determine dirtiness independently if the dirty field wasn't already collected,
                // so that requesting git_diff without dirty still works correctly.
                var isDirty = dirty ?? await GetDirtyAsync().ConfigureAwait(false);
                if (isDirty == true)
                {
    ...
    {
        // --exact-match only returns a tag if HEAD is directly tagged; exits non-zero otherwise.
        // Omit --always so the command fails (returns null) when there is no tag instead of
        // falling back to printing an abbreviated commit hash.
        var tag = await RunGitCommandAsync("describe --tags --exact-match").ConfigureAwait(false);
        return tag?.Trim();
    }
    ...
    internal static async Task<string?> RunGitCommandAsync(string arguments)
    {
        try
        {
            ...
            try
            {
                ...
                // if stderr fills the OS pipe buffer and is never drained, the
                // process blocks waiting to write while we wait to read stdout.
